...
namespace ImpulseClub.Controllers
{
    ...
    public class RecursoController : ControllerBase
    {
        ...
        public RecursoController(IRecursoService service) { _service = service; }
        ...
        public async Task<IActionResult> Create([FromBody] CreateRecursoDto dto)
        {
            ...
            var created = await _service.CreateRecurso(dto);
            return CreatedAtAction(nameof(GetById), new { id = created.Id }, created);
        }

        [HttpPut("{id:guid}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] UpdateRecursoDto dto)
        {
            if (!ModelState.IsValid) return ValidationProblem(ModelState);
            var updated = await _service.UpdateRecurso(dto, id);
